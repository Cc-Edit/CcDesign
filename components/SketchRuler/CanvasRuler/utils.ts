...
export const drawHorizontalRuler = (ctx: CanvasRenderingContext2D, start: number, shadow: shadowType, options: Record<string, any>) => {
  ...
  // 2. 画阴影
  if (shadow) {
    const shadowX = (shadow.x || 0 - start) * scale; // 阴影起点坐标
    const shadowWidth = (shadow.width || 0) * scale; // 阴影宽度
...
export const drawVerticalRuler = (ctx: CanvasRenderingContext2D, start: number, shadow: shadowType, options: Record<string, any>) => {
  const { scale, width, height, canvasConfigs } = options;
  const { bgColor, fontColor, shadowColor, ratio, longfgColor, shortfgColor } = canvasConfigs;
  ...
  if (shadow) {
    // 阴影起点坐标
    const posY = (shadow.y || 0 - start) * scale;
    // 阴影高度

    const shadowHeight = (shadow.height || 0) * scale;
    ctx.fillStyle = shadowColor;
